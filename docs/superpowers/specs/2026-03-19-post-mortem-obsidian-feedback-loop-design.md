...
- `github_issue` (singular integer) is the primary issue — drives `_issues/GH-NNNN.md` hub generation.
- `github_issues` lists all issues processed in the session — makes the report findable from any issue's hub.
...
If a plan has no `## Prior Work` section, the skill creates one before appending.
...
3. **Classify** — review session events:
   - **Blocker**: any task that failed or was retried (detectable from task result metadata or team lead corrective messages recorded in task descriptions)
   - **Impediment**: workarounds, slow-downs, unexpected detours that self-resolved
...
The `## Shut Down` section of `team/SKILL.md` replaces the inline post-mortem writing instructions with a single step:
...
(`superseded_by` is parsed separately via frontmatter, not this regex.)

**`plugin/ralph-knowledge/src/generate-indexes.ts`**

`post_mortem` edges render in issue hubs (`writeIssueHubs`) the same way as `builds_on` — as typed wiki-link edges in the `## Relationships` section. No new index files needed.
...
| `plugin/ralph-hero/skills/team/SKILL.md` | Replace shutdown section with skill invocation |
| `plugin/ralph-hero/hooks/scripts/team-postmortem-completeness.sh` | New hook |
| `plugin/ralph-hero/hooks/scripts/team-shutdown-validator.sh` | Unchanged |
| `plugin/ralph-hero/hooks/hooks.json` | Register new hook |
| `plugin/ralph-knowledge/src/parser.ts` | Add `post_mortem` relationship type |
| `plugin/ralph-knowledge/src/generate-indexes.ts` | Render `post_mortem` edges in issue hubs |
| `plugin/ralph-knowledge/src/__tests__/parser.test.ts` | New relationship tests |
| `plugin/ralph-knowledge/src/__tests__/generate-indexes.test.ts` | New hub rendering tests |
| `.obsidian/graph.json` | Add `type:report` color group |
| `plugin/ralph-hero/skills/ralph-knowledge/setup-obsidian/SKILL.md` | Add report color group |
