...
existing_id=$(gh api --paginate "repos/{owner}/{repo}/issues/$PR_NUMBER/comments" -F per_page=100 \
  --jq "[.[] | select(.body | contains(\"$MARKER\"))] | last | .id // empty" 2>/dev/null | tail -1 || echo "")
