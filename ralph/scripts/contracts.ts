...
export const DELIVER_REASONS = [
  ...
  "awaiting-approval", // GH-2444: the PR's own `reviewDecision` reads REVIEW_REQUIRED — the base ruleset's native approval rule, the same condition merge-pr.sh's gate 1b blocks on. Read straight off the cheap PR-facts fetch every pass, never a merge-gate dry-run probe, so this never enters the marker/retry-window cycle and costs no session. Only a human approving (or the rule changing) clears it
] as const;
