...
That consistency test alone does not catch a missed bump (GH-1976) — it compares the two stamps to *each other*, not to the code. `scripts/check-herdr-version-bump.sh` closes that from the other side: a `pull_request`-only CI job (`herdr-manifest-check`) that validates the manifest is parseable TOML (GH-2431) and reports, informationally, whether the diff touches the plugin's **behavior surface** (`scripts/**`, non-test `cockpit/**`, the manifest itself; docs and tests are excluded). An unresolvable base ref is exit 2, never a pass.

**The stamp itself moves at release time, not per PR (GH-2491).** With 8-12 ralph-herdr PRs open at once, a per-PR bump requirement made the stamp and its changelog-in-the-`description` field the top rebase-conflict magnet in the repo (16 CI failures, 12 rebase conflicts measured 2026-09-02..04). `scripts/herdr-plugin-release-bump.sh`, run from `release-ralph.yml` right after it advances onto main's tip, diffs the behavior surface since the last `ralph-v*` tag and — at most once per release — walks the manifest version and `ralph/scripts/herdr-plugin-version` forward one patch in the same release commit; a release with no herdr behavior change touches neither file. The version history that used to accumulate in the manifest's `description` lives in `plugin/ralph-herdr/CHANGELOG.md`; the `description` is one sentence. Full rationale: `../thoughts/shared/research/2026-09-02-ralph-claude-md-naming-and-install-history.md`.
