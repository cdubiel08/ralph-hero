...
- **Hooks that look up artifacts keyed on `tool_input.file_path` root via `resolve_root_from_path "$file_path"`** (hook-utils.sh), not `get_project_root`. New tests that **invoke a hook** (feed it stdin JSON and assert its exit code) copy `plan-research-required.test.sh`'s harness (SBX/REPO/NOGIT sandboxes + `run_case`). This does NOT apply to doc-structure tests that only grep skill-surface files and drive no hook — those live in the same `__tests__` directory so CI's glob covers them, but sandboxing them would weaken them, since the shipped file IS the subject under test (`caretake-watch.test.sh`, `hero-auto-tick-audience.test.sh`, `hero-watch-envelope.test.sh`, `loop-dispatch-no-double-mode.test.sh`). Gotchas: run cases from a CWD with no `GH-NNN` substring (worktree paths leak into ticket-ID fallbacks), and keep fixture filenames token-free when a case targets a later lookup branch — a `GH-NNN` in the filename satisfies the direct-artifact check first and masks it.
