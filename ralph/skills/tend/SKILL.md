...
- **unformed** — likely raw intake, missing an estimate or a priority (a null priority ranks behind every P3 in `board next`, so an unprioritized item is not deprioritized, it is unreachable — GH-1796): give it an outcome-shaped body, an estimate, a priority (`board priority NNN P2`), and parent/dep wiring where it obviously belongs (`board link`, `board dep`). If it duplicates existing work: comment on both, wire the survivor, and propose the duplicate's closure.
