...
- **PENDING — approval** — the base ruleset's own approval rule is the only gate left (GH-2443; gate 1b runs last, so everything else passed). Record the marker with `gate: "approval"` and exit at In Review, nothing else: no nudge, no escalation — a human approving is the only clear, and from the next pass `board deliver-queue` holds the row as **`awaiting-approval`** (GH-2444) outside the retry window, re-reading `reviewDecision` for free instead of dispatching you again.
...
3. Report, as your final output: `checked`/`acted` counts, the blocked-reason set from the queue read, and the earliest `windowExpiresAt` among time-bounded rows (`settling`, `retry-window`, `deferred`). The lane's goal state — for whatever invoked you — is: empty `next` AND no time-bounded blocked rows; rows only a human can clear (`no-pr`, `convergence-stalled`, `awaiting-approval`, Human Needed) don't count against it. `awaiting-approval` (GH-2444) is a native-approval wait the gate itself reported — a human approving the PR is the only clear, and every pass re-reads `reviewDecision` for free, so it never re-enters the retry cycle. `local-session-active` IS time-bounded — it clears itself on the lock's TTL — so it counts like the others.
