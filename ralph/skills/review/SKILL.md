...
References: [plan-vs-impl-rubric.md](plan-vs-impl-rubric.md) (val rubric: citation gate, drift, integration), [behavior-verification.md](behavior-verification.md) (opus feature close-out stage: UI-heuristic gate, verdict handling), [code-review-prompt.md](code-review-prompt.md) (code-review loop invariants + escalation), [merge-gate.md](merge-gate.md) (pre-merge gates, CI watch, cross-repo, scout report), [auto-vs-interactive.md](auto-vs-interactive.md) (depth-0 fan-out, `RALPH_REVIEW_MODE` switch, fix-cycle bound).
...
2. **Validate** — `Agent(subagent_type="ralph:val-agent", prompt="Validate GH-NNN. Plan doc: ... Follow the validation rubric in ${CLAUDE_PLUGIN_ROOT}/skills/review/plan-vs-impl-rubric.md exactly (citation gate, drift, cross-phase); emit VALIDATION PASS / FIX / FAIL and post the ## Validation Report comment.")`. **Tier routing:** when the plan is a GROUP plan (`github_issues:` frontmatter) or a plan-of-plans, pass `model="fable"` — plan-vs-delivery validation of a feature/epic is a judgment bookend; single XS/S plans keep val-agent's sonnet default. Parse verdict: `VALIDATION PASS` → continue; `VALIDATION FIX` → dispatch impl-agent for mechanical fixes (1 cycle max), re-run val; `VALIDATION FAIL` → STOP `FINISH BLOCKED`.
...
5. **Address feedback** — dispatch `Agent(subagent_type="ralph:impl-agent", prompt="Address PR review feedback for #NNN — Address Mode. Follow ${CLAUDE_PLUGIN_ROOT}/skills/impl/address-mode.md exactly.")`. Wait for return.
...
3. **Pre-merge gates (script-enforced)** — per [merge-gate.md §Pre-merge gates](merge-gate.md): the gates live IN `scripts/merge-pr.sh` (CHANGES_REQUESTED hard block, CI checks green, attestation valid + `head_sha`-bound, external review present, mergeable). Do NOT re-implement them in prose; Step 4's invocation runs them. `merge-review-decision-gate.sh` (PreToolUse:Bash) only funnels bare `gh pr merge` to the script. Scout Report gate unchanged: `closeout-scout-gate.sh` PreToolUse on the merge Bash command. Autonomous mode (`RALPH_AUTO_MERGE=true`) runs the same script gates — on evidence-in-flight failures (pending checks, awaiting external review) STOP and let the next loop tick re-evaluate.
4. **Merge** — `bash scripts/merge-pr.sh PR_NUMBER`. Parse `MERGE GATE PASS` / `MERGE GATE FAIL — <gate>` (`MERGE BLOCKED` is emitted alongside). On success capture `MERGE_SHA` via `gh pr view PR_NUMBER --json mergeCommit --jq '.mergeCommit.oid'`. If the gate blocks on a missing/stale attestation, post it first (default-mode Step 4.9 / [merge-gate.md §Attestation](merge-gate.md)) — standalone merges of un-attested PRs are supposed to block.
