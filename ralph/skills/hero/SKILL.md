...
1. **Detect phase.** Parse `$ARGUMENTS` for an issue number; if none, pick from `next_actions({})` via `AskUserQuestion`. **Audience here is the `human` default — intentional:** this result feeds an interactive `AskUserQuestion` picker (not the autonomous loop), so the agent-only XS/S penalty and Backlog/null-state fallback would mis-rank the human surface. The autonomous queue read lives in `--mode classify` and uses `audience: "agent"`. A picked direction with `kind: "triage"` has no issue number — dispatch `Skill("ralph:caretake", args="--mode triage")` board-wide instead of resolving `TARGET`/`get_issue`. Otherwise, call `get_issue({ number: TARGET, includePipeline: true })` — trust the returned `phase`. State machine + convergence rules in [state-machine.md](state-machine.md).
