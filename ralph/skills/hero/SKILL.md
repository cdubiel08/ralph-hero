...
4. **Dispatch.** If `DISPATCH_REASON` matches `trigger:*` or `RemoteTrigger`, write iOS-mode sentinel: `touch "${TMPDIR:-/tmp}/ralph-ios-mode"`. Then dispatch: for the `blocked:*` tier, `Skill(ENTRYPOINT, args=DISPATCH_ARG)` — i.e. `Skill("ralph:caretake", args="--mode watch-pr")` / `args="--mode watch-upstream")`, a **board-wide watcher sweep** NOT scoped to `NNN` (the watcher modes ignore an issue arg). For all other tiers, `Skill(ENTRYPOINT, args="NNN")` (bare issue number). Unimplemented team (memorykeepers) → emit `needs input: team <name> not yet implemented; skipping dispatch.`
