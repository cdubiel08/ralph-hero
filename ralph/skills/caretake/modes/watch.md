...
| **issue** | Two complementary sweeps, union the results (dedup by issue number): (1) `list_issues(profile: "analyst-unblock", limit: 250)` — the canonical `WAIT-issue=NNN` parking state. Use `analyst-unblock` (`workflowState: "Human Needed"`), **not** `analyst-triage`: that profile means `workflowState: "Backlog"`, and naming a Backlog profile for a Human-Needed sweep is a query that reads as its own opposite. (2) `list_issues(profile: "analyst-triage", limit: 250)` — Backlog, forward-compat for the Gap-A relaxation (a future `WAIT-issue` may park Backlog+edge per triage.md Gap A cross-ref). | For each candidate, identify its blocker(s) from **either** signal (prefer the edge; fall back to the `## Escalation` text): **Edge** — `list_dependencies(number: NNN)` → inspect the `blockedBy` connection, each node carries `number` + `state`; **`## Escalation` body convention (#1472)** — parse the blocker number from `Blocked by #NNN` and the advance target from `Move to <state> once #NNN closes` (default `Ready for Plan` when no explicit target is named). If neither signal yields a blocker number, **skip** the item (not counted). |
