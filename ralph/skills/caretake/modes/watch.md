...
| **issue** | Two complementary sweeps, union the results (dedup by issue number): (1) `list_issues(profile: "analyst-triage", workflowState: "Human Needed", limit: 250)` — the canonical `WAIT-issue=NNN` parking state; (2) `list_issues(profile: "analyst-triage", workflowState: "Backlog", limit: 250)` — forward-compat for the Gap-A relaxation (a future `WAIT-issue` may park Backlog+edge per triage.md Gap A cross-ref). | Identify each candidate's blocker(s) from **either** signal, preferring the edge: **Edge** — `list_dependencies(number: NNN)` → inspect the `blockedBy` connection, each node carries `number` + `state`; **`## Escalation` body convention (#1472)** — parse the blocker number from `Blocked by #NNN` and the advance target from `Move to <state> once #NNN closes` (default `Ready for Plan` when no explicit target is named). If neither signal yields a blocker number, **skip** the item (not counted). |
...
| **issue** | For each all-CLOSED item: 1. **Determine the advance target**: read the embedded condition from the `## Escalation` line (`Move to <state> once #NNN closes`), or an explicit `advance:` hint if present, else **default `Ready for Plan`**. **Bound it to what's legal from the item's ACTUAL current state** (server-validated, GH-1615) — the two source sweeps land in different states with different legal sets: from `Human Needed` the legal targets are `{Backlog, Research Needed, Ready for Plan, In Progress}`; from `Backlog` they are `{Research Needed, Ready for Plan, Done, Canceled}`. If the read/embedded target is not legal from the item's actual current state, fall back to `Ready for Plan` (legal from both sources) and say so in the `## Unblocked` comment ("embedded target `<state>` was not reachable from `<current>`; defaulted to Ready for Plan"). 2. **Remove the dependency edge**: for each `blockedBy` edge found in §Step 3, call `remove_dependency(blockedNumber: <parked-item-number>, blockingNumber: <blocker-number>)`. `blockedNumber` = the parked item; `blockingNumber` = the now-closed blocker. Do NOT use `blockedByNumber` — that param does not exist in the tool schema. 3. `save_issue(number: <parked-item-number>, workflowState: <advance-target>, command: <"ralph_unblock" if advance-target == "In Progress" else "ralph_triage">, labels: <current labels minus any blocked:* and ralph-triage>)` — `ralph_triage` does not allow `In Progress` as a command-level output; only `ralph_unblock` does, so the `In Progress` target needs that command instead. 4. Post a `## Unblocked` comment: "Blocker(s) #NNN closed → dependency edge removed, advanced to `<target>`." (name every blocker number that closed; note the fallback if one fired). |

Every `save_issue` call above passes an **explicit `labels` array** — `save_issue` replaces the full label set; omitting it leaves the stale `blocked:*` label attached and the next sweep re-finds and re-escalates/re-advances the same item (inflating the `ADVANCED <N>` count every tick). `command: "ralph_triage"` (or `"ralph_unblock"` for an `In Progress` target, see the **issue** row above) is passed for semantic parity — transition legality and the command's allowed-output-states are both validated server-side by `save_issue` regardless of which mode is calling it (GH-1615), so an out-of-band target now comes back as a `toolError` naming the legal next states instead of silently writing.
...
- No code changes.
- Rate-limit awareness (**issue** kind): two `list_issues` queries (limit 250 each), then per candidate one `list_dependencies` (or `get_issue`) to find its blockers, **plus one `get_issue` per blocker** (§Step 4 checks EVERY blocker, not just the first) — a meaningful number of API calls per heartbeat tick on a large board; the proactive rate-limiter in the MCP server warns at 100 remaining and blocks at 50.
