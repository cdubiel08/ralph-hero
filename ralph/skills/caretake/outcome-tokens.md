...
- `TRIAGED SPLIT` — **decomposition needed, and delegated to `/ralph:plan --mode epic #NNN`**. Triage creates NO children (GH-1605): `--mode epic` is the single decomposition path and owns child sizing, the one-call `create_sub_issues` batch, the dependency edges, and the three `split-*` hooks. The issue stays in Backlog with the `ralph-triage` label so §Step 2's untriaged-Backlog picker doesn't re-select it. (See `modes/triage.md` § SPLIT — a second child-creation procedure would bypass every one of those gates.)
...
- `ENRICH RECOVERED (PR <url>)` — no file was enriched this pass, but §Step 3's empty-selection check found commits on `chore/enrich-ideas` that `origin/main` does not have — the residue of an earlier pass whose push or PR create failed — and published them (push + idempotent PR create/lookup only; nothing staged, nothing committed). Distinct from `ENRICHED <N> …` because `<N>` would be `0`: the findings in that PR were produced by an earlier invocation.
- `Queue empty.` — no `status: draft` idea files found **and** no unpublished commit on `chore/enrich-ideas`. The second condition is load-bearing: after a push/PR failure, the enriched files are `status: forming` on the branch but still `draft` on `main`, so §Step 2 keeps re-selecting them and §Step 3's post-checkout re-read keeps dropping them. Short-circuiting on the empty selection alone stranded that commit on every subsequent heartbeat.
- `ENRICH SKIPPED — branch <name> is not main` — §Step 1 branch-gate short-circuit (parity with `TRIAGED skipped …`).
- `ENRICH SKIPPED branch-setup-failed: <stage>: <sanitized-stderr>` — §Step 2b's fail-closed branch setup refused before any file was edited. `<stage>` ∈ `fetch-main` | `checkout-local` | `checkout-from-remote` | `checkout-new`, or the post-condition form `on '<branch>', expected 'chore/enrich-ideas'`. Every command that decides which branch the edits land on is exit-checked: an unchecked failure would leave the session on `main`, commit the enrichment there, push the unchanged `$BRANCH` ref, and still report `ENRICHED <N> (PR <url>)` for a PR containing none of the findings. Nothing was edited and no commit exists, so nothing is stranded.
...
- `ENRICH BLOCKED stage-failed: <sanitized-stderr>` — §Step 4 `git add -- "${SELECTED_FILES[@]}"` failed (a selected path vanished, an unreadable file, an index lock held by another process). Checked for the same reason the commit below is: an unchecked `git add` failure leaves the index as it was, so the following `git commit` succeeds against a STALE index and the mode reports `ENRICHED <N> (PR <url>)` for findings that were never staged — while §Step 3 has already flipped those files to `status: forming`, so they are never re-selected. Nothing is committed; the checkout returns to `main`.
- `ENRICH BLOCKED commit-failed: <sanitized-stderr>` — §Step 4 `git commit` failed (pre-commit hook, nothing staged, unconfigured identity, …). The `git commit` was the one unchecked command in an otherwise fully-checked block: a failed commit made the push a no-op that "succeeded", the already-exists fallback still resolved a PR URL, and the mode reported `ENRICHED <N> (PR <url>)` for findings that never reached a PR — while §Step 3 had already flipped those files to `status: forming`, so they would never be re-selected. Checked now; the checkout returns to `main`.
- `ENRICH BLOCKED push-failed: <sanitized-stderr>` — §Step 4 `git push` failed for a non-retryable reason (authentication, permission, protected branch, network, rate limit). These are not branch rejections, the fetch + fast-forward retry cannot repair them, and reporting them as `push-rejected` would hide a credentials/connectivity outage behind a routine-divergence token. Not retried; stderr is carried in the token.
- `ENRICH BLOCKED checkout-main-failed: <sanitized-stderr>` — §Step 4's checked return to `main` failed on some exit path. The enrichment commit is safe on `chore/enrich-ideas`, but the session is still on that branch, so §Step 1's branch gate will short-circuit every subsequent heartbeat until an operator runs `git checkout main`. Emitted instead of claiming recovery from the wrong branch.
- `ENRICH BLOCKED pr-create-failed: <sanitized-stderr>` — §Step 4 `gh pr create` failed for a reason other than an already-open PR, or the already-exists fallback's `gh pr view` failed / returned an empty URL. The commit is pushed; only the PR (or its URL) is missing.

`<sanitized-stderr>` above is never raw `git`/`gh` stderr — it is passed through `modes/enrich.md` § Step 1a's `sanitize_diag()` helper (newlines collapsed, whitespace squeezed, capped at 300 chars) before it reaches the token line. Terminal tokens are single-line and verbatim-parseable by construction (see the file-level invariant above); embedding an un-sanitized multiline diagnostic in one would violate that and could make a harness `grep` for the token miss the final line entirely.
