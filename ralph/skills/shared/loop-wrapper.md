...
| `hero` default | one-shot orchestrator; refuses `--loop`. Use `--auto` → `--mode auto` for the autonomous watcher (never-terminating; idles at a 1h ceiling rather than draining to a stop — see the `hero:auto` row above) |
