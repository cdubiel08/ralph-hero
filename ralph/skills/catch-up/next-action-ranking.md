...
| `lock-stale` | — | **Release the stale claim, then re-dispatch by the released state.** Follow `signals.reclaimInstruction` verbatim — it names the release target for this issue's lock state (e.g. `save_issue(number: NNN, workflowState: "Ready for Plan")` for a stale `Plan in Progress`). Then re-read the row above matching the released state and dispatch that. Do **NOT** route this to `--mode triage`: triage acts only on `Backlog`/null-state items (`caretake/modes/triage.md` § Scope), and a `lock-stale` issue is by definition in a lock state, so a triage dispatch is a guaranteed no-op. |
