...
| `lock-stale` | — | **Release the stale claim, then dispatch the verb for the released state and let that verb's own `__LOCK__` claim be the atomic step.** Follow `signals.reclaimInstruction` verbatim to release (e.g. `save_issue(number: NNN, workflowState: "Ready for Plan")` for a stale `Plan in Progress`), then dispatch per the row above matching the released state. **Do not treat the release as a reservation** — `save_issue` does not check holder identity or claim version, so between release and dispatch another session may legitimately claim the issue. That is handled, not prevented: the dispatched verb opens by claiming the lock, and if it lost the race the claim is refused and it stops. A lost race therefore costs one no-op dispatch, never a double-run. (An atomic reclaim-and-dispatch — a compare-and-set claim token — needs server support; tracked in [GH-1651](https://github.com/cdubiel08/ralph-hero/issues/1651).) Do **NOT** route this to `--mode triage`: triage acts only on `Backlog`/null-state items (`caretake/modes/triage.md` § Scope), and a `lock-stale` issue is by definition in a lock state, so a triage dispatch is a guaranteed no-op. |
