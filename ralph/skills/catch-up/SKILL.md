...
- **Any other mode** (default/narrative/dashboard/brief) → emit refusal from `loop-wrapper.md` § Refusal message, then STOP. `--mode brief` stays refused here even with `--prepare` set — brief is an interactive sitting; `--prepare`'s daily cadence belongs to a scheduled task, not `/loop`.
