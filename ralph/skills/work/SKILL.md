...
4. **Findings outlive the transcript.** A bug, constraint, or stale doc you noticed → `thoughts/shared/research/` note or a linked issue. Deferred work → `board create` + `board dep`, never a TODO comment — pick the lane: `--backlog` when it is ready to be picked up, `--intake` when it still needs a human’s approval.
