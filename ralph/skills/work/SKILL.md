...
7. **Gates are run, not predicted.** Merge through the host repo's own gate: if it ships `scripts/merge-pr.sh`, run `scripts/attest-pr.sh` with real exit codes, then `bash scripts/merge-pr.sh PR`; otherwise use the repo's normal merge flow (PR, green CI, whatever review its policy requires). Either way the gate's verdict is the decision — never simulate, summarize, or pre-judge it, and never demand conventions the repo hasn't adopted (`board readiness` recommends; the repo decides). **Exit 75 / `MERGE GATE PENDING` is not a failure** — evidence isn't in yet (CI still building, reviewer hasn't seen this head). Leave the PR up and the issue In Review; do not escalate, do not `--force`. One push per PR, and attest last: the attestation and the external review are both head-bound, so every extra push re-opens both gates.
