...
- **answer** — for each Human Needed item (`board list --state human`): show its Decision-needed comment, collect the user's answer (AskUserQuestion, one per item, recommendation first), post it as a comment, then `board move NNN <backlog|in-progress>` per the answer. An item reopened by `reconcile` because it was closed as an unevidenced **apply unit** asks a narrower question — *was the change actually deployed?* If yes, the answer is evidence (`scripts/apply-evidence.sh`), not a state move; if it will never be deployed, `board cancel`. Do not simply re-close it.
