...
- **new "<title>"** — crystallize the user's description into `board create --title … --body …` (body: outcome + acceptance in the user's words, no template ceremony). Pass `--priority` when known — an item without one sorts *after* every prioritized item in `board next`. `board priority NNN <option>` corrects one already filed. Offer `--parent`/`board dep` wiring when it obviously belongs to existing work.
