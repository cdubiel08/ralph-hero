...
- **proposals** — `/ralph:tend` files closure proposals as `<!-- ralph-tend:v1 proposed -->` marker comments on Backlog items; they re-surface as the `proposed` category of `board tend-queue` and as doctor's `tend-proposal-stale` line. Disposing of one is a human call: `board cancel NNN -m` (superseded), `board move NNN done --why "<how it was delivered>"` (already delivered — the Done evidence gate still applies), `board reopen NNN`, or leave it in Backlog to reject. Say what you decided in a comment, so the next pass reads a decision rather than re-proposing.
