...
- **new "<title>"** — crystallize the user's description into `board create --title … --body …` (body: outcome + acceptance in the user's words, no template ceremony). Always pass `--priority` — an item filed without one sorts *after* every prioritized item in `board next`, so an unprioritized issue is effectively invisible. `board priority NNN <option>` corrects one already filed. Offer `--parent`/`board dep` wiring when it obviously belongs to existing work.
