...
**A comment counts as the marker only if it has the generated body shape** — a `## Decision Request` heading followed by at least one `#### Decision:` block whose text matches an open decision block in the plan doc. A bare heading, an empty section, or a heading whose blocks match nothing in the plan is **not** the artifact this check is looking for; ignore it and fall through to the `Absent` branch. Without that shape test any comment containing the literal string parks the plan at `PLAN AWAITING DECISION` indefinitely. (Author identity is deliberately *not* the test: ralph posts through the operator's own token — `RALPH_HERO_GITHUB_TOKEN`, defaulting to `gh auth token` — so the generated comment and a hand-written one carry the same GitHub author, and an author check would reject the real marker as readily as a fake one.)
