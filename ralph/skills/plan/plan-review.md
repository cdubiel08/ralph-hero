...
**Two targeted reads are permitted inside this pre-check — and only these two.** The pre-check is cheap because of what it *skips* (the full plan read, the rubric pass, the critique sub-agent), not because it reads nothing:

1. **The plan's open decision blocks.** Grep the plan doc for its `#### Decision:` headings and the `status:`/resolution line under each; do not read the phases, files-affected tables, or verification steps. This is what the shape test below compares against. `plan/SKILL.md` § `--mode review` Step 3 names the same targeted read, so the two contracts describe one procedure.
2. **The critique doc for this issue** under `thoughts/shared/reviews/` — its frontmatter `decisions_open` and `decision_request_comment` only.

**A comment counts as the marker only if ALL THREE of the following hold:**

- **Identity (the authorization test)** — the comment's GraphQL **node ID equals the critique doc's `decision_request_comment` frontmatter value** (read 2). That value is written by the hold branch below from the `create_comment` response, so it names *the one comment ralph actually generated* — not any comment shaped like it. No other comment on the issue can satisfy it, because an attacker would have to make GitHub mint a node ID that a doc committed through the merge gate already records.
- **Shape** — a `## Decision Request` heading followed by at least one `#### Decision:` block whose text matches an open decision block in the plan doc (read 1). Retained as a cheap pre-filter and as an integrity check on the identified comment: if the recorded comment was edited into a bare heading or an empty section, it no longer carries the question the human is being asked, so treat it as `Absent` and re-post. Without the shape test any comment containing the literal string parks the plan at `PLAN AWAITING DECISION` indefinitely.
- **Corroboration** — that same critique doc has `decisions_open > 0`. This is what says a hold is still owed at all; the identity test says *which comment* is the hold.

**Why identity, not shape plus "some critique exists".** The previous pair of conditions authenticated the *situation*, not the *artifact*: once any critique for the issue carried `decisions_open > 0`, any commenter who could read the plan could copy its open decision blocks under a `## Decision Request` heading and satisfy both tests, parking auto review at `PLAN AWAITING DECISION` on every tick. Binding to the recorded node ID closes that: shape is reproducible, a node ID minted for ralph's own comment is not. (CodeRabbit, PR #1620, 2026-07-28.)

**Recording the node ID.** The hold branch below captures `create_comment`'s returned comment node ID and writes it into the critique doc frontmatter as `decision_request_comment: <node-id>` in the same pass, before emitting `PLAN AWAITING DECISION`. The critique doc is the right home for it because it is the trusted artifact already in the loop: it reaches `main` through the merge gate (attested PR + CI + external review, GH-1589), and it is agent-written — nothing on the issue surface can edit it.

**Back-compat / recovery.** A critique doc predating this field has `decisions_open > 0` and no `decision_request_comment`. There is then no authenticated marker, so the check falls to `Absent` and the hold is re-posted once — which records the node ID and makes every subsequent tick idempotent. `review-no-dup.sh` blocks a duplicate critique write, so this costs one comment, not a re-critique.

(GitHub *author* identity is deliberately not the test: ralph posts through the operator's own token — `RALPH_HERO_GITHUB_TOKEN`, defaulting to `gh auth token` — so the generated comment and a hand-written one carry the same GitHub author. An author check would reject the real marker as readily as a fake one; the recorded node ID discriminates where the author field cannot.)
...
- Absent (no comment, malformed shape, no recorded node ID, or a comment whose node ID does not match the recorded one) → the critique doc from read 2 already tells you which case this is. If it exists with `decisions_open > 0`, the real marker was edited, deleted, never recorded (pre-field critique), or is being impersonated: re-post the `## Decision Request` from the plan's open blocks WITHOUT re-critiquing (`review-no-dup.sh` would block a duplicate critique write anyway), **overwrite `decision_request_comment` with the new comment's node ID**, then emit `PLAN AWAITING DECISION`. If it does not exist, no hold was ever issued — proceed to the critique dispatch below.
...
The sub-agent's output IS the critique doc (with the workflow body fixing the frontmatter + filename — including `decisions_open: <n>`, the count of open `#### Decision:` blocks at review time, and, once a hold is posted, `decision_request_comment: <node-id>`, the GitHub node ID of the `## Decision Request` comment that hold generated). Auto mode never opens an AskUserQuestion picker.
...
| APPROVED with open decisions (auto) | None — stays Plan in Review | `## Decision Request` comment (its node ID recorded in the critique's `decision_request_comment`) + PushNotification, once; emits `PLAN AWAITING DECISION` |
