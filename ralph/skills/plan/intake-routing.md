...
- **`--mode epic`** — `ARG` SHOULD be `#NNN` of an epic-tier issue (M/L/XL or `kind:epic` labeled). Free-form descriptions work but produce a plan-of-plans not linked to GitHub state. **This mode now has an effective input-state precondition** (GH-1615): the epic must be reachable to the `Plan in Progress` lock via the promote hop through `Ready for Plan` — `SKILL.md`'s `--mode epic` Step 1 runs the shared Legal claim path fragment rather than locking unconditionally, and an epic sitting at `In Progress` / `In Review` / a terminal state has no legal claim path at all.
