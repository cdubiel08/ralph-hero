...
4. **The session split ledger** (`hook-utils.sh` `split_ledger_*`) — the classification, the parent number, the recorded parent estimate, and the permitted-attempt flag that arms `split-postcondition.sh` at Stop. Written by `split-size-gate.sh` from the real create payload and the tool's own per-child status report.

**No env var participates in the classification or the arming** — not even additively. `RALPH_SUBCOMMAND=epic-split` was briefly retained as a "tightening-only" signal for `split-size-gate.sh` and has been removed (CodeRabbit, PR #1620, 2026-07-28): tightening off a *stale* value is still mis-arming. A leftover `RALPH_SUBCOMMAND=epic-split` exported from an operator's shell profile forced the atomic `{XS,S}` ceiling onto a genuine plan-of-plans batch (every legitimate S/M child refused as "estimate too large"), and armed `split-postcondition.sh` for unrelated `/ralph:plan` sessions that have no ledger count — so Stop blocked them as unverifiable. It never armed `split-estimate-gate.sh` either: that value can be present before the session starts, so arming a `get_issue` blocker with it would enforce *before* Step 0 has classified anything (GH-1603 round 9).
