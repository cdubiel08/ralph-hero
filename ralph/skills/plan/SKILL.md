...
2. **Select issue** — `ARG=#NNN` → `get_issue`; else `list_issues(profile: "analyst-plan", limit: 50)`, filter XS/S Ready-for-Plan + unblocked, pick highest priority. None eligible → exit cleanly. (XS/S do not require linked research — the gate waives sub-threshold work.)
...
Strategic multi-tier decomposition. Writes a plan-of-plans doc + creates feature children with dependency edges.
...
Critique an existing plan and emit APPROVED / NEEDS_ITERATION. Consult `plan-review.md`.
