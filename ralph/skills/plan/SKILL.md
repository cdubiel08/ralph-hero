...
7. **Commit through a PR** — branch, `git add <the plan doc>`, commit `docs(plan): GH-NNN implementation plan`, `gh pr create`, then `bash scripts/attest-pr.sh PR_NUMBER` and `bash scripts/merge-pr.sh PR_NUMBER`, return to `main`. Full recipe: `../shared/artifact-commit.md`. **Never `git push origin main`** (ruleset-protected, GH-1589) and never bare `gh pr merge` — `bash scripts/merge-pr.sh PR_NUMBER` is the only merge path. A PR the gates hold open is not a failed run — report its URL and continue.
...
3. **Held-plan short-circuit (auto only)** — if `RALPH_REVIEW_PLAN=auto`, run the idempotency check BEFORE the full plan read, the rubric pass, or any critique dispatch (the auto tick re-fires review on every held plan every pass — this step keeps those ticks cheap). Two **targeted** reads are permitted inside it and are what make the check decidable: the plan's `#### Decision:` blocks only, and the critique doc's `decisions_open` frontmatter. An existing `## Decision Request` comment counts only if it matches the plan's open blocks AND is corroborated by that critique doc — full contract in `plan-review.md` § Interactive vs auto. Then: emit `PLAN AWAITING DECISION` or fold a human reply. Held with no answers → STOP here.
