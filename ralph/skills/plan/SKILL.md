...
4. **Parent-plan reuse** — per `intake-routing.md` § Parent-plan reuse (matches parent plan-of-plans AND existing sibling group plans). If short-circuit: post `## Plan Reference`, advance child to "In Progress", report, STOP.
...
6. **Write plan doc** — per `plan-shapes.md` (auto-column required sections, including Files Affected; group: § Group plans — `github_issues:` frontmatter, one phase per member). Stamp `estimate:` into the frontmatter from the fetched issue (group: the HIGHEST member estimate). The `plan-research-required.sh` hook blocks Write if no linked research AND the estimate is ≥ threshold (the `estimate:` stamp waives sub-threshold XS/S); `doc-structure-validator.sh` blocks Stop if required sections missing.
