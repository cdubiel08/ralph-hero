...
7. **Commit through a PR** — branch, `git add <the findings doc>`, commit `docs(research): GH-NNN research findings`, `gh pr create`, then `bash scripts/attest-pr.sh PR_NUMBER` and `bash scripts/merge-pr.sh PR_NUMBER` (both take the PR number and are invoked through `bash` — a bare `scripts/merge-pr.sh` with no argument is not the mandated form), return to `main`. Full recipe: `../shared/artifact-commit.md`. **Never `git push origin main`** (ruleset-protected, GH-1589) and never bare `gh pr merge`. A PR the gates hold open is not a failed run — report its URL and continue.
