...
**Model tiers (GH-1593) are not landed yet.** The pins above are hand-maintained literals in skill/agent frontmatter. A `.ralph-models.yml` capability-tier registry (`cheap`/`standard`/`capable`/`frontier`), its per-surface renderer, and the `scripts/check-model-tiers.sh` CI drift-check land in a separate PR; until then there is no config to derive pins from and no drift gate on them.
...
| `RALPH_IMPL_MODEL` | No | Override the model for `impl-agent`. Takes a raw Claude Code model id/alias (`haiku`, `sonnet`, `opus`, `fable`), used as-is. Defaults to `sonnet`; BLOCKED escalation re-dispatches once at `opus`. Capability-tier names are GH-1593 and not available yet. |
