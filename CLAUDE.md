...
**Model tiers are config, not just convention (GH-1593).** Every pin above is *derived from* `.ralph-models.yml` (repo root) — a `cheap`/`standard`/`capable`/`frontier` capability-tier map, rendered per-surface (skill frontmatter accepts `best`; agent frontmatter and `Agent(model=...)` dispatch params render the concrete literal instead). `scripts/check-model-tiers.sh` (CI) fails on any hand-edited pin that drifts from the config, including new pin sites added without a manifest entry. `hero-fable`'s pin and the `IMPL BLOCKED needs=opus` escalation-chain literals are explicit hard pins — allowlisted, never tier-derived.
...
| `model-tier-registry.ts` | `.ralph-models.yml` Zod schema + pure loader + `resolveTier()` (GH-1593); no registered tool consumer yet — the typed reference implementation, checked against the same shipped YAML as `scripts/model-tiers/render.js` |
...
| `RALPH_IMPL_MODEL` | No | Override the impl dispatch tier. Accepts EITHER a `.ralph-models.yml` tier name (`cheap`/`standard`/`capable`/`frontier` — resolved via the config) OR a raw Claude Code model id/alias (`sonnet`, `opus`, `fable`, `haiku` — used as-is, legacy behavior). Defaults to `standard` (= `sonnet`); BLOCKED escalation re-dispatches once at `opus`. |
| `CLAUDE_CODE_SUBAGENT_MODEL` | No (harness-native, not ralph plumbing) | Escape hatch for the `model: fable` pins on `plan-agent`/`review-agent`: set to `opus` on accounts without Fable. Top precedence in Claude Code's subagent model resolution — it overrides frontmatter AND per-invocation `model` params, so it flattens EVERY subagent tier (locators, impl ladder, BLOCKED re-dispatch). The `.ralph-models.yml` tier config sits below this in the resolution order and has no lever over it. Leave unset if your account has Fable. |
