...
function initGitHubClient() {
    ...
    const repoToken = process.env.RALPH_GH_REPO_TOKEN ||
    ...
    const projectNumber = process.env.RALPH_GH_PROJECT_NUMBER
        ...
        : undefined;
    if (!owner || !repo) {
        console.error("[ralph-hero] Warning: RALPH_GH_OWNER and/or RALPH_GH_REPO not set.\n" +
            "Most tools require these. Set them in your environment or .claude/ralph-hero.local.md");
    }
    if (!projectNumber) {
        console.error("[ralph-hero] Warning: RALPH_GH_PROJECT_NUMBER not set.\n" +
            "Project-level tools (workflow state, field updates) will not work.\n" +
            "Run /ralph-setup to configure your GitHub Project.");
    }
    const repoTokenSource = process.env.RALPH_GH_REPO_TOKEN ? "RALPH_GH_REPO_TOKEN" : "RALPH_HERO_GITHUB_TOKEN";
        ...
        if (projOwner && projNum) {
            try {
                ...
                for (const ownerType of ["user", "organization"]) {
                    try {
                        const result = await client.projectQuery(`query($owner: String!, $number: Int!) {
