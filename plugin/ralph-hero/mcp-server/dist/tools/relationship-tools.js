...
function resolveConfig(client, args) {
    ...
    if (!owner)
        throw new Error("owner is required (set GITHUB_OWNER env var or pass explicitly)");
    if (!repo)
        throw new Error("repo is required (set GITHUB_REPO env var or pass explicitly)");
    return { owner, repo };
}
// ---------------------------------------------------------------------------
// Register relationship tools
// ---------------------------------------------------------------------------
export function registerRelationshipTools(server, client, _fieldCache) {
    // -------------------------------------------------------------------------
    ...
    server.tool("ralph_hero__add_sub_issue", "Create a parent/child (sub-issue) relationship between two GitHub issues. The parent issue becomes the container for the child issue.", {
        ...
        try {
            ...
            const result = await client.mutate(`mutation($parentId: ID!, $childId: ID!, $replaceParent: Boolean) {
            addSubIssue(input: {
              issueId: $parentId,
              subIssueId: $childId,
