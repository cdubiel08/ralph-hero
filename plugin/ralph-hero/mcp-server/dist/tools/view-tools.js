...
// Register view tools
// ---------------------------------------------------------------------------
export function registerViewTools(server, client, fieldCache) {
    // -------------------------------------------------------------------------
...
async function fetchViews(client, owner, number) {
    const VIEWS_QUERY = `
    query($owner: String!, $number: Int!) {
      OWNER_TYPE(login: $owner) {
        projectV2(number: $number) {
  ...
  `;
    // Try user
    try {
        const result = await client.projectQuery(VIEWS_QUERY.replace("OWNER_TYPE", "user"), { owner, number }, { cache: true, cacheTtlMs: 60 * 1000 });
    ...
    // Try org
    try {
        const result = await client.projectQuery(VIEWS_QUERY.replace("OWNER_TYPE", "organization"), { owner, number }, { cache: true, cacheTtlMs: 60 * 1000 });
