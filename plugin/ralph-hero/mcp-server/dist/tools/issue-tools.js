...
async function fetchProjectForCache(client, owner, number) {
    ...
    for (const ownerType of ["user", "organization"]) {
        try {
            const result = await client.projectQuery(QUERY.replace("OWNER_TYPE", ownerType), { owner, number }, { cache: true, cacheTtlMs: 10 * 60 * 1000 });
...
// ---------------------------------------------------------------------------
async function resolveIssueNodeId(client, owner, repo, number) {
...
// ---------------------------------------------------------------------------
async function resolveProjectItemId(client, fieldCache, owner, repo, issueNumber) {
    ...
    const result = await client.query(`query($issueId: ID!) {
      node(id: $issueId) {
        ... on Issue {
    ...
    if (!projectItem) {
        throw new Error(`Issue #${issueNumber} is not in the project (projectId: ${projectId}). ` +
...
// ---------------------------------------------------------------------------
async function updateProjectItemField(client, fieldCache, projectItemId, fieldName, optionName) {
    const projectId = fieldCache.getProjectId();
    if (!projectId) {
    ...
    await client.projectMutate(`mutation($projectId: ID!, $itemId: ID!, $fieldId: ID!, $optionId: String!) {
...
// ---------------------------------------------------------------------------
async function getCurrentFieldValue(client, fieldCache, owner, repo, issueNumber, fieldName) {
...
function resolveConfig(client, args) {
    ...
    if (!repo)
        throw new Error("repo is required (set RALPH_GH_REPO env var or pass explicitly)");
    return { owner, repo };
}
function resolveFullConfig(client, args) {
...
// ---------------------------------------------------------------------------
export function registerIssueTools(server, client, fieldCache) {
    // -------------------------------------------------------------------------
    ...
    server.tool("ralph_hero__create_issue", "Create a GitHub issue and add it to the project with optional field values (Workflow State, Estimate, Priority)", {
        ...
        try {
          ...
          }`, { owner, repo }, { cache: true, cacheTtlMs: 60 * 60 * 1000 });
            ...
            const createResult = await client.mutate(`mutation($repoId: ID!, $title: String!, $body: String, $labelIds: [ID!], $assigneeIds: [ID!]) {
            createIssue(input: {
              repositoryId: $repoId,
              title: $title,
              body: $body,
    ...
    // -------------------------------------------------------------------------
    server.tool("ralph_hero__update_workflow_state", "Change an issue's Workflow State in the project (e.g., 'Backlog' -> 'Research Needed')", {
        owner: z.string().optional().describe("GitHub owner. Defaults to GITHUB_OWNER env var"),
        repo: z.string().optional().describe("Repository name. Defaults to GITHUB_REPO env var"),
        number: z.number().describe("Issue number"),
