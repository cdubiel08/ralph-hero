 ...
 */
export async function detectGroup(client, owner, repo, seedNumber) {
...
// Helpers
// ---------------------------------------------------------------------------
function addIssueToMap(map, data) {
 ...
 */
function filterGroupMembers(issueMap, seedNumber) {
...
function topologicalSort(issueMap, groupNumbers) {
    ...
    if (sorted.length !== groupNumbers.length) {
        ...
        throw new Error(`Cycle detected in dependencies! Issues involved: ${cycleMembers.map((n) => `#${n}`).join(", ")}. ` +
            `These issues form a circular dependency chain. Remove one dependency to resolve.`);
    }
    return sorted;
}
//# sourceMappingURL=group-detection.js.map