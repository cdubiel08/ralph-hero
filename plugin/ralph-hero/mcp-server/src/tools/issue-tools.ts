...
import { diceSorensen, extractSearchKeywords } from "../lib/similarity.js";
...
// ---------------------------------------------------------------------------
// Helper: Score and rank duplicate candidates (pure, exported for testing)
// ---------------------------------------------------------------------------
...
export function scoreCandidates(
  ...
  for (const c of candidates) {
    const score = diceSorensen(targetTitle, c.title);
    if (score < threshold) continue;

    // Extract workflow state from project field values
    let workflowState: string | null = null;
    if (c.projectItems?.nodes) {
      for (const item of c.projectItems.nodes) {
        for (const fv of item.fieldValues.nodes) {
          if (
            fv.__typename === "ProjectV2ItemFieldSingleSelectValue" &&
            fv.field?.name === "Workflow State"
          ) {
            workflowState = fv.name ?? null;
          }
        }
      }
    }
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        const targetIssue = issueResult.repository.issue;
        if (!targetIssue) {
          return toolError(`Issue #${args.number} not found in ${owner}/${repo}`);
        }

        // Step 2: Extract search keywords
        const keywords = extractSearchKeywords(targetIssue.title, targetIssue.body);
        if (!keywords) {
          return toolSuccess({
        ...
        // Step 3: Build and execute GitHub search
        ...
        // Step 4: Score, filter, and rank candidates
        const rawCandidates = searchResult.search.nodes
          .filter((n) => n.number && n.number !== args.number);
