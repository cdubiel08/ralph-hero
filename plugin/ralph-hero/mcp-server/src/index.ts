    ...
    async () => {
      ...
      if (projOwner && projNum) {
        try {
          ...
          let project: { title: string; fields: { nodes: Array<{ name: string }> } } | null = null;

          for (const ownerType of ["user", "organization"]) {
            try {
              const result = await client.projectQuery<Record<string, {
                projectV2: { title: string; fields: { nodes: Array<{ name: string }> } } | null;
              }>>(
