...
- Weekly `near_misses` in the run state (GH-2259) counts paraphrase near-misses **recorded**, never suppressed — nothing branches on it. It is the measurement #1965's deferral is waiting on, and the readings are not interchangeable: an absent `<wiki_dir>/_near_misses.jsonl` means never instrumented, a `kind: "scan"` record with `near_misses: 0` means instrumented and no churn — but only once at least one scan compared a candidate against a known axiom (a `failed`/`empty` run's `candidates: 0` scan proves the instrument ran, not that the corpus is clean; the report says NOT CERTIFIABLE until then, GH-2283), and non-zero means #1965's trigger has fired. Read the accumulation with `uv run scripts/dream/meta_reflect.py --near-miss-report` (from the repo root; this skill already runs the dream scripts by repo path).
- `near_misses: null` in `dream-meta-state.json` (GH-2283) is a THIRD reading, distinct from both: it means THIS run's scan write itself failed (an `OSError` — disk full, permissions), so this run left no evidence either way. It is not the same as `0` (ran clean, found nothing) and not the same as the file being wholly absent (never instrumented at all — a different, older run). `meta_reflect.py` logs a WARNING and `dream_health.warn_if_uninstrumented` surfaces it, but never fails the run over it — a missing datum may not cost a real weekly run.
