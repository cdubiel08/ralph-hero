...
| `work-team` | split (down) | TEAM LAUNCH (GH-2178, narrowed in GH-2214, staffing revised in GH-2461): prompts for the epic, then `work-team.sh EPIC` spawns or respawns the standing read-only o-lane LEAD (`o<EPIC>-<slug>`, role `orchestrator`) idempotently — a live `o<EPIC>-*` is never doubled — then, uncontained, staffs the epic-scoped INITIAL fleet and arms the watcher (`work-fleet.sh --epic EPIC --refill`) before stopping. The lead owns its team's workers but no longer spawns them: GH-2266's process containment denies the lead's own pane write access to the checkout it sits in, so its `git fetch` could never succeed there — refill.sh (triggered by `watch-event.sh`) keeps the team staffed from EPIC's frontier as workers exit; worker spawn records still carry the lead's ref as C8 parent/root lineage. The lead rehydrates from board state alone, so a dead one is respawned by re-running the command (`--lead-only` is accepted for compatibility and names the same, only behavior) |
| `team-stand-down` | split (down) | STAND DOWN a live lead deliberately (GH-2357): prompts for the epic, then `work-team.sh EPIC --stand-down` appends the durable `{ev: exit, reason: "stood-down"}` fact for the live lead's ledger ref under the ledger mutex and only THEN closes the team workspace — so the pane-death event this produces finds the ref already closed and the healer respawns nothing, and `rh day` (`resume-teams.sh`) never resumes it. The park lever for an epic that is open but has nothing agent-staffable left (before it, `/exit` in that lead's pane read as a crash and was respawned within seconds, forever). No live lead is a clean no-op; an unreadable herd or a lead with no open ledger ref refuses and closes nothing. Re-arm with `work-team` |
...
| `dispatch-up` | split (down) | DISPATCH UP (GH-2213, D3.1; placement amended by GH-2246): `dispatch-up.sh` ensures the repo's main workspace exists (the source checkout whose linked worktrees form the fleet), the hero pane is up **in it**, and then prints the roster best-effort. The dispatch address remains `<repo>/dispatch`, minted by `board name dispatch`; only the pane placement changed. Re-run HEALS, never refuses: a standing space is reused, a live hero sitting is left alone, a dead one reopened. It arms nothing scheduled — no cron, no rota; the unattended half of the lane is `watch-event.sh` + `heal.sh` (GH-2212). It does stamp the dispatch heartbeat (D7.2), so running doctor's named remedy is itself the heartbeat write. Failure directions, stated: the address and workspace reads fail closed (a second main workspace is unsafe); hero liveness and the roster print fail open (the cost is a duplicate disposable pane / a missing printout) |
...
| `e` | **epic popover** (GH-2381, spec §11) — the selected card's parent epic with every child as a board card, two columns in board order (In Progress, In Review, Backlog, Human Needed, Done), the child's board STATE in line 1's right slot; the title row carries closed/total, the children's summed cost and a state tally. One `board get <epic> --json` on the press (children ride the issue fetch), re-read on the signal cadence while it is up; a Done child's closing PR is joined from the Done-window read, dispatched once when the epic has one. `j`/`k` move between children; `Enter` / `Space` / `g` observe, peek and open the SELECTED CHILD through the board's own verbs (a peek's `esc` returns to the popover); `esc` closes. Offered in the legend only on a parented card; on any other, `e` refuses on the status line |
...
**Epic-scoped arming (GH-2461)**: `work-fleet --epic EPIC --refill` — the
team lead's staffing path, run *uncontained* by `work-team.sh` at team
launch, since the lead's own contained pane cannot fetch — records EPIC in
`fleet.json`'s `epic` field. Every later refill pick stays inside EPIC's
frontier only (`board frontier --json --epic EPIC`), never the whole
board's. Omit `--epic` and arming is unscoped exactly as before.
...
~/.ralph/<owner>/<repo>/runs/<run_id>/     # run_id = UTC timestamp + 4 hex
  fleet.json           arming state: {run_id, armed, k, refill, budget_left,
                       expires_at, repo, epic, spawned: [...], inflight: [...]}
                       (armed is written as refill==1 — a refill=0 arm is a
                       disarmed audit record no watcher ever refills; epic is
                       null unless armed via --epic — GH-2461)
...
- **A stood-down lead is not healed (GH-2357).** `work-team.sh EPIC
  --stand-down` closes the lead's ledger ref (`exit reason=stood-down`)
  BEFORE closing its workspace, so the death event finds no open ref for
  the pane and this path never runs — no exit reappend, no orphan flag, no
  respawn. `heal.sh` keeps a second guard on the same fact for any caller
  that hands it a ref without re-proving it open. A stood-down team is
  re-armed by a human running `work-team.sh EPIC` again; `resume-teams.sh`
  never does it.
