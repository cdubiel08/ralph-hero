...
| `dispatch-up` | split (down) | DISPATCH UP (GH-2213, D3.1): `dispatch-up.sh` ensures the named `<repo>/dispatch` workspace exists (label from `board name dispatch` — the address is the board's to mint, GH-2209, never spelled here), the hero pane is up **in it** (a hero standing in some other workspace does not satisfy it), then prints the roster best-effort. Re-run HEALS, never refuses: a standing space is reused, a live hero sitting is left alone, a dead one reopened. It arms nothing scheduled — no cron, no rota; the unattended half of the lane is `watch-event.sh` + `heal.sh` (GH-2212). It does stamp the dispatch heartbeat (D7.2), so running doctor's named remedy is itself the heartbeat write. Failure directions, stated: the address and workspace reads fail closed (a second `<repo>/dispatch` is the one duplicate this command exists to prevent); hero liveness and the roster print fail open (the cost is a duplicate disposable pane / a missing printout) |
