...
**Board finding (tool defect)**: the plan-of-plans' claim that the dependency chain "is wired on the board" is TRUE — but `ralph_hero__get_issue` reported empty `blockedBy`/`blocking` for every child. Root cause: `get_issue` maps `blocking`/`blockedBy` from the legacy task-list `trackedInIssues`/`trackedIssues` connections (`issue-tools.ts:998-1003`), not the `blockedBy` connection that `add_dependency`'s `addBlockedBy` mutation writes. Confirmed by direct GraphQL (edges present) and by all eight `add_dependency` re-adds failing with "Target issue has already been taken". This read/write asymmetry sits squarely in the files this wave edits and deserves a fix or a spun-off issue.
...
| 11 | Plan-of-plans: dependency chain "wired on the board" | **VERIFIED — but exposed a `get_issue` defect** | `get_issue` returned `blockedBy: []` for all six children, yet direct GraphQL shows all 8 edges present (#1610/#1611/#1612 ← #1609; #1614 ← #1610,#1611,#1612,#1613; #1612 ← #1603) and re-adding each fails "already been taken". `get_issue` reads `trackedIssues`/`trackedInIssues` (task-list tracking) instead of the `blockedBy` connection `add_dependency` writes — `issue-tools.ts:998-1003` |
...
3. **Cross-feature edge #1612 ← #1603 is real and already wired on the board.** #1603 (sibling feature #1590, currently Research Needed) decides `collate_debug`'s consumer fate and also deletes `caretake/modes/trends.md` (a #1611 consumer) and `catch-up --mode dashboard` (a #1610 consumer of `pipeline_dashboard.formatted`). If #1590's wave runs in parallel, #1610/#1611 will edit files #1603 deletes — coordinate, or accept that whichever lands second re-does the consumer sweep. Caveat: `get_issue` cannot see these edges (defect above); orchestrators relying on `get_issue.blockedBy` will treat #1612 as unblocked — use `list_dependencies` until fixed.
...
0. **`get_issue` blocked-edge misread** (found incidentally): should the `trackedIssues`→`blockedBy` fix ride this wave's PR (it touches `issue-tools.ts` anyway) or spin off as its own issue? It affects any consumer trusting `get_issue.blockedBy` — including pipeline convergence checks.
