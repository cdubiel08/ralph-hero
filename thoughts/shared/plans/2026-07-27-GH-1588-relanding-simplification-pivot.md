...
### 2026-07-31 — PR-B round 1: 19 findings, and a cross-PR leak the equivalence check missed

CodeRabbit round 1 on [PR #1647](https://github.com/cdubiel08/ralph-hero/pull/1647): **19 actionable, CHANGES_REQUESTED**. Comparable to PR-A's round 1 (12), so within the convergence budget — the watch triggers on a *non-decaying* rate across 3+ rounds, not on round-1 volume.

**12 fixed, 7 answered with evidence.** Three findings were factually wrong (a test assertion the reviewer said was missing but demonstrably runs; a harness "port" that is a category error — static-grep doc tests have no artifact resolution to sandbox; and one I got wrong myself, below). Three are real but already filed as Phase 4 follow-ups (#1649/#1650/#1651) — those were filed *before* the review landed, which made the disposition a pointer rather than a deferral. Two more were genuinely pre-existing and are now tracked in #1652.

**The finding that matters: a cross-PR documentation leak.** `hero/dispatch.md` documented a `cheap|standard|capable|frontier` capability-tier table resolved through `.ralph-models.yml` — **a file that does not exist on this branch.** It, its renderer, and `check-model-tiers.sh` are PR-C material. The tier vocabulary was non-functional as shipped: anyone following the docs would have passed an unrecognized model id.

Sweeping for the same class found it was wider than the flagged line — root `CLAUDE.md` described the tier config as live in two places, listed a `model-tier-registry.ts` lib module absent from this branch, and documented the PR-C CI check as active; `README.md` carried tier names in the env table.

This violated **this plan's own cross-PR invariant** (§ Testing Strategy: *"main is releasable after every merge — each PR is self-consistent"*), and my equivalence check missed it because I applied Phase 4 §2's diff to `mcp-server/` and `scripts/` but never to `ralph/` **prose**. **Lesson for PR-C and any future layer slice: the equivalence assertion must also grep the docs for forward references to artifacts the current PR does not carry.** A layer slice cuts code cleanly but documentation describes the whole system, so prose is where the slice leaks.

**A disposition I got wrong, and corrected.** I dismissed the `research/SKILL.md` branch-gate finding as "pre-existing, frontmatter untouched by this PR." Re-checking rather than trusting my own summary showed this PR *had* edited that frontmatter (removing the GH-1619-demoted hook registrations) *and* rewritten the exact prose line carrying the false claim. The prose listed `branch-gate.sh` among the frontmatter gates while the frontmatter carries an explicit comment that it is deliberately **not** registered. Corrected the prose to describe the real mechanism (Step 1's own `git branch --show-current`) and posted a public correction to the disposition. Worth recording because the failure mode — dispositioning from memory of what I changed instead of from `git diff` — is repeatable.

**Operational note on the CodeRabbit quota, superseding both earlier notes.** Phase 1 said "never wait, always re-request." The PR-B entry above corrected that to "request once, then wait." Refining once more with the observed data: the plan quota is **exhausted for this repo right now**, and each request returns a *sliding* window (40 → 25 → 35 minutes observed). A review does still land when a window opens — round 1 arrived that way. Practical rule: **one request per opened window, never two**, and treat "Full review finished" with a Fair-Usage footer as *no review was produced*, distinct from "Full review triggered" (which does produce one).
