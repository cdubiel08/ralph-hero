...
- [ ] `bash ralph/hooks/scripts/__tests__/impl-plan-required.test.sh` exits 0, all cases pass
- [ ] `find ralph/hooks/scripts/__tests__ \( -name '*.test.sh' -o -name 'test-*.sh' \) -type f -print0 | sort -z | xargs -0 -n1 bash -c 'echo "=== $0 ==="; bash "$0"'` (CI's `test-hooks` job invocation) passes for the full hook-test suite
- [ ] `shellcheck` at `severity: error` (CI's `shellcheck-hooks` job, `scandir: ralph/hooks`) passes on `ralph/hooks/scripts/impl-plan-required.sh`

#### Manual Verification
- [ ] From a shell CWD'd outside any repo with `CLAUDE_PROJECT_DIR` unset, invoke the hook directly with a crafted PreToolUse JSON whose `file_path` points into a real repo checkout's source tree for a ticket with an existing plan doc under that repo's `thoughts/shared/plans/` — confirm exit 0
...
#### Automated Verification
- [ ] `bash ralph/hooks/scripts/__tests__/review-no-dup.test.sh` exits 0, all cases pass
- [ ] `find ralph/hooks/scripts/__tests__ \( -name '*.test.sh' -o -name 'test-*.sh' \) -type f -print0 | sort -z | xargs -0 -n1 bash -c 'echo "=== $0 ==="; bash "$0"'` (CI's `test-hooks` job invocation) passes for the full hook-test suite
- [ ] `shellcheck` at `severity: error` (CI's `shellcheck-hooks` job, `scandir: ralph/hooks`) passes on `ralph/hooks/scripts/review-no-dup.sh`
