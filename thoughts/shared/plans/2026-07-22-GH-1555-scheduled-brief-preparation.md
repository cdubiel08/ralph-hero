...
- **Setup** — the `sed -e "s|__HOME__|...|g" ...` render command writing to `~/Library/LaunchAgents/com.$(whoami).brief-prepare.plist`, followed by `launchctl load`.
- **Verify** — `launchctl list | grep brief-prepare` should show the label loaded.
