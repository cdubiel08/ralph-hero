...
**Post-review addendum (PR #1573 code review, 2026-07-21):** `searchRepoIssues` originally returned `Promise<RepoSearchIssueNode[]>` with no truncation signal — a single-page fetch (`first: $limit`, no pagination) that silently dropped any matches beyond `limit`, reintroducing the same silent-truncation bug class this plan exists to close. Fixed by also reading `search.issueCount` from the GraphQL response and changing the return type to `Promise<RepoIssueSearchResult>` (`{ nodes, truncated, totalCount }`), where `truncated = totalCount > nodes.length`. Both call sites below were updated accordingly; see the addendum at the end of this document for the full fix description.
...
**Post-review addendum:** the response now also folds in `incomplete: true`, `totalCount`, and a `warning` string when `searchRepoIssues()` reports `truncated: true` — i.e. when GitHub reports more total matches than were returned within `limit`. When the result is exhaustive, those three fields are omitted entirely (not `false`/`null`) so existing consumers checking only `filteredCount`/`items` see no shape change.
...
**Post-review addendum:** a match found within the fetched page still wins outright (returns `toolError` regardless of `truncated`). But when `searchRepoIssues()` reports `truncated: true` (more than `limit=10` open issues matched the title-based search) AND no match was found in the fetched page, the dedup check is now incomplete rather than exhaustive — a real duplicate could rank outside the top 10 hits and go undetected. Rather than silently falling through to creation as before, this case now also returns a `toolError` (naming `totalCount` and instructing the caller to narrow the title further or pass `skipDedupeCheck: true`). This is distinct from the search-failure fallback path immediately below: a *failed* search still falls through to creation (best-effort, per the Design Decision above), but a *truncated-but-successful* search does not, since silently treating a partial existence check as exhaustive is exactly the failure mode this plan exists to close.
...
## Post-Merge Addendum: truncation signal (PR #1573 code review, 2026-07-21)

The shipped Phase 2/3 code (as merged) had `searchRepoIssues` do a single-page fetch (`first: $limit`) with no truncation signal — matches beyond `limit` were silently dropped with no indication to either caller. Automated code review flagged this as a MUST_FIX (100/100 confidence): it reintroduces the exact silent-truncation bug class this plan's Phase 2 already fixed on the project-scope path (commit `a4ecc18b`), and is most dangerous in `create_issue`'s dedup check — a real duplicate ranking outside the top 10 search hits would go undetected and silently create a duplicate, one layer deeper than the incident this plan documents.

Fix, addressed in the same PR:
- `searchRepoIssues` (`mcp-server/src/lib/repo-issue-search.ts`) now also reads `search.issueCount` from the GraphQL response and returns `Promise<RepoIssueSearchResult>` (`{ nodes, truncated, totalCount }`) instead of `Promise<RepoSearchIssueNode[]>`. `truncated = totalCount > nodes.length`.
- `list_issues`'s `scope: "repo"` branch (`issue-tools.ts`) now folds `incomplete: true` / `totalCount` / a `warning` string into its response when `truncated` is true, omitting all three when the result is exhaustive (no shape change for existing consumers).
- `create_issue`'s dedup pre-check (`issue-tools.ts`) now returns a `toolError` (rather than falling through to creation) when `truncated` is true and no exact match was found in the fetched page — the highest-severity half of the review finding, since a false "no duplicate" verdict on an incomplete check is worse than a false positive.
- Both call sites, `repo-issue-search.test.ts`, `list-issues-scope.test.ts`, and `create-issue-dedup.test.ts` were updated for the new return shape, with new test cases covering the truncated-result path for both callers. Full suite: `npm test` passes (1654 tests, 1 pre-existing skip) and `npm run build` exits 0.
