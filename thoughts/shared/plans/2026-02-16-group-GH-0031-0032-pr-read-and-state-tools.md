...
- [x] Existing `create_pull_request` still works (resolveConfig import change is compatible)
- [x] `get_pull_request` returns detailed PR with reviews, checks, linked issues
- [x] `list_pull_requests` returns filtered list with compact summaries
