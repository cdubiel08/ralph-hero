...
| D6.2 | **Address is the title, everywhere** (panes, labels, cards, ledger) | Amended by GH-2235: the ADDRESS is absolute, the DISPLAY is the shortest unambiguous suffix given its container — labels drop the segments the container already renders (repo, and the team inside a team space); keep the TAIL, middle-truncate only what remains. The pre-amendment guidance ("truncate middle, keep repo + tail") rendered two identical truncated rows for different agents. Amended again by GH-2320 for the COCKPIT CARD only: its line 2 is the issue title, not the address display — the card already names the unit on its branch line and the team on its epic chip, so the suffix there was the same number+slug twice and displaced the one line saying what the unit is about; the full address stays on the topology view. Panes, labels and the ledger keep the address |
