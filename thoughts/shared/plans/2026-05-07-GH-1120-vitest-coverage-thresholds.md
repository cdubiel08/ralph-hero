...
- [ ] `npm test` in `plugin/ralph-hero/mcp-server/` exits 0 and prints a coverage summary table.
- [ ] `npm test` in `plugin/ralph-knowledge/` exits 0 and prints a coverage summary table.
- [ ] `pnpm test` in `plugin/ralph-demo/remotion/` exits 0 and prints a coverage summary table.
- [ ] Each package contains `coverage/lcov.info` after a test run.
- [ ] Decreasing any threshold value by 1 in any config (then re-running) causes vitest to exit non-zero with a clear "X% < threshold" message. (Manual sanity check; revert before commit.)
...
- **depends_on**: null
- **acceptance**:
  - [ ] `package.json` lists `@vitest/coverage-v8` in `devDependencies` at the same major as `vitest` (`^4.0.18` ⇒ `^4`).
  - [ ] `package-lock.json` is regenerated by running `npm install` (no manual edits).
...
- **depends_on**: null
- **acceptance**:
  - [ ] `package.json` lists `@vitest/coverage-v8` in `devDependencies` at the same major as `vitest` (`^4.0.0` ⇒ `^4`).
...
- **depends_on**: null
- **acceptance**:
  - [ ] `package.json` lists `@vitest/coverage-v8` in `devDependencies` at the same major as `vitest` (`^4.1.5` ⇒ `^4`).
  - [ ] `pnpm-lock.yaml` regenerated via `pnpm install` (NOT `npm install` — this package uses pnpm).
  - [ ] `package.json` `scripts.test` is `vitest run --coverage`.
  - [ ] `package-lock.json` is NOT created in this directory.
...
- **acceptance**:
  - [ ] In `plugin/ralph-hero/mcp-server/`: run `npm test` (which now includes `--coverage` from Task 1.1) and capture lines/functions/branches/statements percentages from the printed table.
  - [ ] In `plugin/ralph-knowledge/`: run `npm test` and capture the four percentages. (Baseline runs with default config since `vitest.config.ts` is created in Task 1.6.)
  - [ ] In `plugin/ralph-demo/remotion/`: run `pnpm test` and capture the four percentages.
  - [ ] Record the 12 measured values (3 packages × 4 metrics) for use in Tasks 1.5, 1.6, 1.7.
  - [ ] If any package's tests fail under coverage instrumentation (e.g., timing-sensitive tests breaking with the ~20% slowdown), record the failure and stop — flag for human review before locking thresholds.
...
- **depends_on**: [1.4]
- **acceptance**:
  - [ ] `coverage` block added under `test` with: `provider: "v8"`, `reporter: ["text", "json-summary", "lcov"]`, `include: ["src/**/*.ts"]`, `exclude: ["src/__tests__/**", "src/**/*.d.ts"]`.
  - [ ] `thresholds` object includes `lines`, `functions`, `branches`, `statements` — each set to (Task 1.4 baseline value − 2 to − 3 percentage points), rounded down to a whole integer.
  - [ ] Each threshold value has an inline trailing comment in the form `// 2026-05 baseline N%` documenting the measured baseline.
  - [ ] `npm test` exits 0.
...
- **depends_on**: [1.4]
- **acceptance**:
  - [ ] New file `plugin/ralph-knowledge/vitest.config.ts` exports a `defineConfig` from `vitest/config`.
  - [ ] `test.include` preserves the package's existing test discovery (e.g., `["**/*.test.ts"]` or matches whatever vitest's default would have picked up — verify by running `npm test` after creation and confirming the same number of test files are picked up as before Task 1.2).
  - [ ] `test.coverage` block follows the same shape as Task 1.5: `provider: "v8"`, three reporters, include/exclude appropriate to this package's source layout (`src/**/*.ts` likely), four thresholds at baseline − 2 to − 3 points with inline date comments.
  - [ ] `npm test` exits 0 and discovers the same test files as before.
...
- **depends_on**: [1.4]
- **acceptance**:
  - [ ] Existing `test` block preserved verbatim (`environment: "jsdom"`, `globals: true`, `include: ["src/**/*.test.{ts,tsx}"]`, `setupFiles: ["src/test-setup.tsx"]`).
  - [ ] New `coverage` sub-block added under `test`: `provider: "v8"`, `reporter: ["text", "json-summary", "lcov"]`, `include: ["src/**/*.{ts,tsx}"]`, `exclude: ["src/**/*.test.{ts,tsx}", "src/test-setup.tsx", "src/**/*.d.ts"]`.
  - [ ] Four thresholds at baseline − 2 to − 3 with inline date comments.
  - [ ] `pnpm test` exits 0.
...
- **acceptance**:
  - [ ] `plugin/ralph-hero/mcp-server/coverage/lcov.info` exists after `npm test`.
  - [ ] `plugin/ralph-knowledge/coverage/lcov.info` exists after `npm test`.
  - [ ] `plugin/ralph-demo/remotion/coverage/lcov.info` exists after `pnpm test`.
  - [ ] `coverage/` is in `.gitignore` for each package, OR added in this PR if missing (verify before committing — accidentally checking in `coverage/` artifacts is a common regression).
...
- **depends_on**: [1.5, 1.6, 1.7]
- **acceptance**:
  - [ ] In one package, temporarily decrease the `lines` threshold by 1 (e.g., 72 → 73 if baseline is 75), run tests, confirm vitest exits non-zero with a clear "X% < threshold" message.
  - [ ] Revert the change before committing the PR.
  - [ ] Document the sanity-check outcome in the PR description.

### Phase Success Criteria

#### Automated Verification:
- [ ] `npm run build` (in `plugin/ralph-hero/mcp-server/`) — no errors. Sanity check that vitest config changes don't break tsc.
- [ ] `npm test` (in `plugin/ralph-hero/mcp-server/`) — passes, prints coverage summary, emits `coverage/lcov.info`.
- [ ] `npm test` (in `plugin/ralph-knowledge/`) — passes, prints coverage summary, emits `coverage/lcov.info`.
- [ ] `pnpm test` (in `plugin/ralph-demo/remotion/`) — passes, prints coverage summary, emits `coverage/lcov.info`.
