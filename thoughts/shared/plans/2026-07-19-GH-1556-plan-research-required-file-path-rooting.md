...
Also add one negative-control case confirming a target file with NO `.git` ancestor anywhere in its path still falls back to `$SBX` (i.e., existing `CLAUDE_PROJECT_DIR`-only behavior is preserved when there's no repo boundary to find) — reuses the existing `$PLANS` fixture under `$SBX` with the existing GH-1 doc, run once more to confirm the fallback path is exercised, not just early-allow paths that never reach `resolve_root_from_path`.
