...
**Changes**: (a) Add `captured: YYYY-MM-DDTHH:MM:SSZ` to the draft template frontmatter (below `status: draft`); (b) new § "Idea-file lifecycle contract" documenting the shared D→C interface: `status: draft` (capture) → `forming` (enrichment) → `refined` (form "keep as refined idea" path, pre-existing); `captured` written at capture; `enriched` + `## Enrichment` section written by the enrichment pass (`caretake --mode enrich`); the daily brief (#1553) is the downstream reader. Note that multi-thought dumps yield N files, each carrying its own `captured`.
